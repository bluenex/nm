...
import ora from 'ora';
import { findNodeModulesWithSizes, formatBytes, NodeModulesInfo } from '../utils/nodeModules';
...
export async function rmCommand(targetPath: string): Promise<void> {
  try {
    ...
    const totalSelectedSize = selectedDirs.reduce((sum, dir) => sum + dir.size, 0);
...
async function removeDirectories(directories: NodeModulesInfo[]): Promise<void> {
  ...
  for (let i = 0; i < directories.length; i++) {
    ...
    try {
      ...
      spinner.stop();
      throw new Error(`Failed to remove ${dir.path}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
